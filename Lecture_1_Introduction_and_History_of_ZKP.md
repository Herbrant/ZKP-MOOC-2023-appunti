...
La *view* generata dal simulatore è identica (Perfect Zero Knowledge) a quella generata in un'interazione reale tra Prover e Verifier.
...
#### Esempio di Rewinding technique
Presentiamo l'esempio di Extractor nel caso delle claim $y$ quadrato residuo modulo $N$.

**Extractor**(y, N)
1. Run prover and receive $s = r^2$ mod $N$
2. Set verifier message to *head*; receive and store $r$
3. **Rewind** and set Verifier message to *tail*, receive and store $rx$ mod $N$
